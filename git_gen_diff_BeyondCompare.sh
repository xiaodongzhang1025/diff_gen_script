...
target_git_diff_dir=~/Diff/$cur_git_dir
if [ ! -z $2 ] ; then
...
function gen_diff() {
  ...
  do
    ...
    if [ -z "$result" ] ; then
      ...
      echo "Step 1. Gen diff dir $target_git_diff_dir."
      
      cp -r $diff_dir_temp/$line $target_git_diff_dir
  ...
  do
    ...
    if [ "$line_prefix" = "??" ] ; then
      #echo $tmp_name
      if [ -f $tmp_name ]; then
        cp $tmp_name $target_git_diff_dir/right/$tmp_name
      else
        cp -r $tmp_name $target_git_diff_dir/right/$tmp_name
      fi
    fi
  done
}
